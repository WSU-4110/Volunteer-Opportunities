...
import {
  ListingInterface,
  ListingsProps,
} from "@/app/explore/(components)/Userpage";
...
export async function addTalentToListing(listingID: string, skillID: string) {
  await database
    .insert(skillsToListings)
    .values({ skillId: skillID, listingId: listingID });
}
...
export async function insertTalent(listingID: string, talent: string) {
  ...
  await database
    .insert(skillsToListings)
    .values({ skillId: talentID, listingId: listingID });
}
